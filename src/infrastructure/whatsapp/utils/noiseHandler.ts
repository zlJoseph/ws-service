...
import { CertChain, CertChain_NoiseCertificate_Details, HandshakeMessage } from "../proto";
...
export const makeNoiseHandler = ({
	...
	const localHKDF = (data: Uint8Array) => {
		...
		return [Uint8Array.prototype.slice.call(key, 0, 32), Uint8Array.prototype.slice.call(key, 32)];
	}
	...
	return {
		...
		mixIntoKey,
		finishInit,
		processHandshake: ({ serverHello }: HandshakeMessage, noiseKey: KeyPair) => {
            ...
            if(!certIntermediate || !certIntermediate.details){
                throw new Boom('certification null', { statusCode: 400 })
            }

            const { issuerSerial } = CertChain_NoiseCertificate_Details.decode(certIntermediate.details)
